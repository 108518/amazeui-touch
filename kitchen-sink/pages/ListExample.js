...
const img = <img width="32" src="http://www.placecage.com/100/100" />;
const img80 = <img width="80" src="http://www.placecage.com/180/180" />;
const img40 = <img width="48" src="http://www.placecage.com/130/130" />;
const badge1 = (
...
function handleSwitch(e) {
  // get checkbox value
  console.log(this.field.checked);
}
  ...
  render() {
    return (
      <Container {...this.props}>
        <Group header="静态列表" noPadded>
          <List>
            <List.Item role="header">A</List.Item>
            <List.Item title="列表项 1" />
            <List.Item title="列表项 2" after={badge1} />
            <List.Item
              title="列表项 3"
              after={
            ...
            />
            <List.Item title="列表项 4" after={mySwitch} />
            <List.Item
              title="列表项 4"
        ...
        <Group header="包含链接的列表" noPadded>
          <List>
            <List.Item href="#" title="列表项 1" />
            <List.Item href="#" title="列表项 2" />
            <List.Item href="#" after="2015.08" title="列表项 3" />
          </List>
        </Group>

        <Group header="包含图标的列表" noPadded>
          <List>
            <List.Item media={img} after={badge1} title="列表项 1" />
            <List.Item media={img} after="2015.08" title="列表项 2" href="#" />
            <List.Item media={img} after={badge1} title="列表项 3" href="#" />
          </List>
        </Group>

        <Group header="含描述的文字列表" noPadded>
          <List>
            {albums.map((album, i) => {
              return <List.Item {...album} target="_blank" key={i} />;
            })}
          </List>
        </Group>

        <Group header="图文列表" noPadded>
          <List>
            {albums.map((album, i) => {
              return (
                <List.Item
        ...
        <Card title="标题">
          <List>
            <List.Item after={badge1} title="列表项 1" />
            <List.Item
              after={
                <Badge rounded amStyle="success" text="ok" />
              }
